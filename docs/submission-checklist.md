...
- [ ] Keep this Mac awake and online (runner + tunnel under caffeinate),
      or migrate to a VPS: ops/launch.md has the two-command migration.
      After any tunnel restart: update manifests' x402.endpoint + push.
- [ ] Check agents weekly: `tail apps/agents/data/*.log.jsonl`; breakers
      clear by deleting the halted flag in apps/agents/data/<name>.state.json.
