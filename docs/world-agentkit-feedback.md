...
Written 2026-09-12 while adding AgentKit to Agripinaa for ETHOnline 2026.
What we built: the eight first-party agent wallets are registered in
AgentBook by their operator; the marketplace shows a "Human-backed · World ID"
badge resolved live from AgentBook on each profile; the runner's paid
...
payment applies. Code: `apps/agents/src/agentkit-gate.ts`,
...
- The prize brief says to test with the World ID Sandbox App. The CLI has no
  sandbox or staging flag; the only alternative network is `base-sepolia`. It
  is not documented whether a `base-sepolia` registration uses the staging
  World ID that the Sandbox App can verify, or production World ID. We could
  not test the registration step without a person holding World App.
