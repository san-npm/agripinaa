...
- **NEVER run an external autonomous agent (codex, or any CLI that executes its own shell commands) against this working repo.** On 2026-08-24 a codex review run reset HEAD from `d19555c` back to `11051da`, silently un-committing a finished task while another agent was mid-work. The reflog entry was a bare `reset: moving to 11051da`, and a `.audit-XXXXXX` scratch directory from the pashov skill's `mktemp -d ./.audit-XXXXXX` appeared and vanished in the repo root. `--sandbox read-only` did NOT prevent it. Nothing was lost (the commit stayed reachable via reflog and `git reset --hard d19555c` restored it), but the failure is silent and could easily have been mistaken for an agent's own mistake. Run such tools against a disposable clone instead: `git clone --no-hardlinks /Users/hodlmedia/agripinaa <scratch>/review-clone`, which costs about 10 MB and takes a second. Use `--no-hardlinks` so a destructive git operation in the clone cannot reach shared objects.
