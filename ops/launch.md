...
| `LP_RANGE_VENUE` | VM only, optional. `pancakeswap-v3` (default) or `uniswap-v3`: which concentrated-liquidity venue Ranger runs its own capital on (`apps/agents/src/lp-venues.ts`, addresses probed on-chain and recorded there). Own-capital state is kept per venue, so switching starts a fresh position on the new venue and leaves the old one where it is until you switch back. Managed mandates are authorized for PancakeSwap only: while another venue is selected the runner logs `managed-skip` for each Ranger mandate and moves nothing. |
