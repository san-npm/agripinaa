...
| `LP_RANGE_VENUE` | VM only, optional. `pancakeswap-v3` (default) or `uniswap-v3`: which concentrated-liquidity venue Ranger runs its own capital on (`apps/agents/src/lp-venues.ts`, addresses probed on-chain and recorded there). Managed mandates keep the PancakeSwap session policy regardless. |
