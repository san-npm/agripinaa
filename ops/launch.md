...
| `LP_RANGE_VENUE` | VM only. `uniswap-v3` (production) or `pancakeswap-v3` (the original venue, and what an unset value still means): which concentrated-liquidity venue Ranger runs its own capital on (`apps/agents/src/lp-venues.ts`, addresses probed on-chain and recorded there). Own-capital pool and position state is kept per venue, so switching starts a fresh position on the new venue and leaves the old one where it is until you switch back; a pending Ophis order and the weekly budget stay shared. Managed mandates are bound to the venue their session policy names (`RANGER_POSITION_MANAGER`, Uniswap v3) whatever this says. |
| `AGENTKIT_PUBLIC_HOSTS` | VM only, optional. Comma-separated hosts the World AgentKit challenge may be bound to, on top of the tunnel hostname the runner reads from `ops/tunnel-url.txt` (written by `report-runner-url.sh` and `start-agents.sh`) and local hosts. A request with any other Host gets the plain payment challenge and no AgentKit invitation. |
| `GRAPH_API_KEY` | Optional. A Subgraph Studio API key (free tier is enough). `GRAPH_GATEWAY_BASE` exists only so tests can point the key at a stub; never set it in production, since the key is sent as a bearer to whatever host it names. With it the marketplace lists and reads agents from Agent0's ERC-8004 subgraph on The Graph Network first, labeled `the-graph`, and falls through to 8004scan and the snapshot as before. Without it the lane is skipped. The same variable in the VM's `ops/ops.env` turns on the yield agents' second-opinion read of Venus and Aave supply rates from Messari's standardized lending subgraphs (`apps/agents/src/graph-rates.ts`): a rotation the chain justifies must also be justified there, and a stale or unreachable subgraph leaves the chain's decision standing. |
