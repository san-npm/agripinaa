...
| `LP_RANGE_VENUE` | VM only, optional. `pancakeswap-v3` (default) or `uniswap-v3`: which concentrated-liquidity venue Ranger runs its own capital on (`apps/agents/src/lp-venues.ts`, addresses probed on-chain and recorded there). Own-capital pool and position state is kept per venue, so switching starts a fresh position on the new venue and leaves the old one where it is until you switch back; a pending Ophis order and the weekly budget stay shared. Managed mandates are bound to PancakeSwap whatever this says, since that is what their session policy authorizes. |
